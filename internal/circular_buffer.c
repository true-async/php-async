...
zend_result circular_buffer_realloc(circular_buffer_t *buffer, size_t new_count)
{
	...
	if (buffer->head >= buffer->tail) {
		...
		if (EXPECTED(new_count > buffer->capacity)) {
			 ...
			 *            tail  head
			 */
			void *new_data = (buffer->allocator->m_realloc)(
					buffer->data, new_count * buffer->item_size, buffer->capacity * buffer->item_size);
			...
			memcpy(new_data, (char *) buffer->data + buffer->tail * buffer->item_size, count * buffer->item_size);
