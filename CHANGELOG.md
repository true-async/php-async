...
- **#125 chaos coverage: `ThreadPool` counters + `Channel`/`ThreadChannel` capacity** — new `fuzzy-tests/thread_pool/counters.feature` (`getWorkerCount`/`getPendingCount`/`getRunningCount`/`getCompletedCount` drained-snapshot invariants) and `fuzzy-tests/channel/capacity.feature` (`Channel::capacity`/`ThreadChannel::capacity`). Overall coverage 127/167 (76%).
