...
### Added
- **#129 I/O chaos: write-path back-pressure** — `EvilPeer` gains a consume mode (`EvilPeer::consume()`): instead of serving a payload it reads from the client slowly, or not at all. New steps `an evil peer "X" that never reads` / `... that reads N bytes at a time`, `delays N ms between reads`, `stops reading after N bytes`, `holds the connection for N ms`, and a client `uploads N bytes to peer "X"` (optionally `in M-byte writes`). New feature `fuzzy-tests/io/backpressure.feature`: uploads larger than the loopback send buffer so `fwrite()` is forced to suspend on the reactor write-wait hook — slow-drain, never-read + cancel, peer-abandons-mid-stream, and a scheduler-chaos crossing. Caught a real reactor bug (see Fixed).
