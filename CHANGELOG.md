...
### Fixed
- **`spawn_thread` SEGV when transferred closure binds `$this`** — closure was recreated in the worker with `this_ptr=NULL` while op_array still carried `ZEND_ACC_USES_THIS`; `FETCH_OBJ_R UNUSED` then dereferenced `EX(This).value.obj` as a NULL object. `async_thread_check_op_array` now rejects such closures with `Error: closure binds $this` before the snapshot is built (recurses into nested `dynamic_func_defs`).
