...
### Fixed
- **`iterate()` ran one callback past the concurrency limit and silently cancelled the last one still working.** The coroutine that drives the iteration loop runs the callback itself, but was never counted in `active_coroutines`, while every coroutine decremented that counter on exit. `concurrency: N` therefore allowed `N + 1` callbacks at once, and the count reached zero one worker early: the iterator completed, and `cancelPending` (on by default) cancelled the callback still running with `AsyncCancellation` — so a callback slower than its peers was lost without a trace. The driver now takes a slot like any other worker, and a slot is released on both ways out (after iterating, or on dispose if the coroutine was cancelled before it ever started), so the last coroutine to leave is the one that completes the iterator.
