...
- **#125 chaos coverage: `TaskSet` join API** — new `fuzzy-tests/task_set/joins.feature` covers `joinAll`/`joinNext`/`joinAny` (incl. the all-fail `CompositeException` path). New harness support: `defineTaskSet()` plus a teardown dispose. `TaskSet` API coverage now 14/14; overall 139/167 (83.2%).
