...
### Added
- **#127 I/O chaos: EvilPeer foundation** — new `fuzzy-tests/_peers/EvilPeer.php`, a deliberately misbehaving network peer driven by a declarative fault table. Toxics so far: payload slicing, inter-chunk drip delay, and abrupt mid-stream connection close (`reset`). New chaos topic `fuzzy-tests/io/` — `evil_peer.feature` verifies the async reactor reassembles a sliced/dripped byte stream exactly; `abrupt_close.feature` verifies a dropped connection yields a clean payload prefix and the client never hangs. Harness gains `defineEvilPeer()` + a prep-phase that binds each peer's listening socket and serves it from a coroutine.
