...
- **#125 chaos coverage: `TaskGroup` result accessors** — new `fuzzy-tests/task_group/getters.feature` covers `spawnWithKey` (incl. duplicate-key throw), `getResults`/`getErrors` partitioning, `suppressErrors`, the `getIterator` foreach path and the direct-call guard. `TaskGroup` API coverage now 17/17; overall 121/167 (72.5%).
