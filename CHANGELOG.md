...
### Notes
- `Scope::disposeAfterTimeout()` leaves its internal timer coroutine live after the timer fires and the scope is finished — it is not reaped within the harness drain window. The `disposeAfterTimeout` chaos scenario therefore omits the "no orphan coroutines" invariant. Potential leak worth a separate look.
