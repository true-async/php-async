...
### Fixed
- **#154 `ThreadPool` worker crash on `exit()`/`die()` in a task or bootloader** — a graceful `exit()`/`die()` (or a fatal error) inside a submitted task or the bootloader threw an unwind-exit token that the worker either passed to `reject()` or re-raised via `zend_bailout()` — both crash the worker fiber with `"Error transfer requires a throwable value"` (assert + ASAN stack-use-after-return at `zend_fibers.c:491`). Fixed in `thread_pool.c`: the sync task call and the bootloader call now run under a `zend_try`, and after the call the worker checks `zend_is_unwind_exit()`/`zend_is_graceful_exit()` on `EG(exception)`. A bailout/exit is converted into an `Async\ThreadTransferException` delivered to every pending task's awaiter (the pool "receives" the bailout), the worker exits cleanly instead of re-raising into the fiber, and the coroutine-mode dispose path got the same unwind-token guard. Regression tests `tests/thread_pool/061-bootloader_exit.phpt`, `062-task_exit_sync.phpt`. (Coroutine-mode task `exit()` resolves the future to `null` — graceful success — which is intended; surfacing the worker-exit separately is tracked as follow-up.)
