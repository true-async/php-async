...
- **A closed handle returned its descriptor to the process while a thread-pool worker still named it** (#282). `libuv_io_close()` closed `crt_fd` and woke every parked coroutine with `Stream was closed`, but a `uv_fs_read`/`uv_fs_write` handed to the thread pool goes on running: the next `open()` got the number back and the worker moved that file's offset — `Assertion 'lseek(self->fd, 0, 1) == 0' failed` in `_php_stream_fopen_from_fd` on a debug build, someone else's file read or written on a release one — while the woken coroutine freed the buffer the worker was still reading, which ASAN reports as a heap-use-after-free of the whole write buffer — `092-filter_remove_during_write.phpt` on an ASAN build shows the 3.2 MB read by `uv__fs_write` in a worker thread, and `086-close_during_io.phpt` the 8 KB read buffer written by `uv__fs_read`. Both are clean after the change; the ASAN run of `ext/async/tests` plus php-src `file`, `streams` and `filters` is 1935 tests with one failure, the local network artefact. `async_io_t` now counts the requests the thread pool holds (`fs_in_flight`); a close with a non-zero count leaves the descriptor open and notifies the waiters without an error, so they park again, and the last completion callback closes the descriptor and releases them. php-src hands the descriptor over in `php_stdiop_close()` for a file handle with no `FILE*` of its own, and keeps the stream's read buffer for the same span: `php_stream_free()` freed it in the same call as `ops->close`, so the worker went on writing into freed memory even while the coroutine stayed parked — valgrind counted 20 such writes in one run of `086-close_during_io.phpt` and none after. Measured on `tests/io/086-close_during_io.phpt` with 300 rounds and four processes at once: 5 aborts in 60 runs before, 0 in 260 after, and 0 in 40 on a release build. A cancelled read is the one wake left on the old route: `uv_cancel` cannot preempt a running worker, so the coroutine leaves with the request still in flight and the read buffer is freed by the next `fclose` — #286.

- **`096-socket_read_and_write.phpt` failed on Windows before it asserted anything.** `stream_socket_pair(STREAM_PF_UNIX, ...)` answers WSAEINVAL there. It takes the pair from `create_socket_pair()`, which picks the INET domain on Windows. Test only.

- **A second `Async\Pool::__construct()` orphaned the first pool and re-targeted the object.** PHP lets a constructor be called again on a live object, and this one overwrote `obj->pool` with no release of what was there: the first pool leaked with its resources in it — the debug allocator names the `ecalloc()` in `zend_async_pool_create_internal()` — and the object went on answering for the new pool. On the wrapper `PDO::getPool()` caches, that meant `idleCount()` reporting a stranger's pool and `tryAcquire()` handing out whatever the new factory built, while the PDO handle itself kept working, so nothing said the view was wrong. A second construction throws `Pool is already initialized` now, before any callback is copied.
- **`PDO::getPool()` handed userland a pool of raw C pointers, and `release()` on it segfaulted.** A pool the engine builds keeps in its slots what C put there — `pdo_pool_factory()` writes each connection with `ZVAL_PTR`, phpredis writes a socket the same way — while `Async\Pool` is the class PHP builds for its own values. `$pool->release(42)` reached `pdo_pool_before_release()`, which does `Z_PTR_P(resource)` and read `conn->conn_broken` through the address 42: SIGSEGV from ordinary PHP with an argument of the declared type. `acquire()` and `tryAcquire()` took a connection out of the idle buffer and answered null on a release build and died on the return-type check on a debug one; either way nothing put the connection back, and the counters went from `idle=2 active=0` to `idle=1 active=1` per call. All three now throw `PoolException` when the pool's factory is internal, and `acquire()` checks before taking anything. Counting is untouched: `count()`, `idleCount()`, `activeCount()` and `getState()` carry no slot value, and reading them is what `PDO::getPool()` is for.
- **Seven methods of `Async\Pool` read through a pool the object did not have.** `unserialize('O:10:"Async\Pool":0:{}')` builds the wrapper without its constructor, and `getState()`, `setCircuitBreakerStrategy()`, `activate()`, `deactivate()` and `recover()` then dereferenced NULL — one line of ordinary PHP, exit 139. They answer `Pool is not initialized` now, as `acquire()` and `release()` already did.
- **A pool still alive when the object store is freed ended the process with SIGSEGV.** `zend_async_pool_close()` builds a `PoolException` for the waiters it wakes, and `async_new_exception()` answers NULL once `EG(active)` is gone — which is exactly the phase `zend_objects_store_free_object_storage()` runs in, so a pool reached by the engine's own free rather than by a destructor released a NULL. Nothing waits in that phase, the coroutines that could wait having been destroyed before it, so the wake is now skipped along with the exception. The shape that reaches it from PHP is a pool held in a reference cycle, and phpredis reaches it from `redis_pool_destroy()`: a suite that leaves one pooled client behind reported every test as passed and then exited 139. Evidence: `tests/pool/030-pool_freed_at_shutdown.phpt`. A pool freed that late still cannot run a PHP `destructor` callback — `zend_call_function()` refuses with the executor down — so a resource whose cleanup matters should be released before the request ends rather than left to the pool's own free.
...
- **A write libuv cancelled during reactor shutdown built a PHP exception after the object store was gone** (#264). `zend_deactivate` runs `shutdown_executor()` before `ZEND_ASYNC_ENGINE_SHUTDOWN()`, so the store is freed and `EG(active)` is 0 by the time `libuv_reactor_shutdown` turns the loop to finish cancelling work. `io_pipe_writev_cb` branched on `status == 0` alone and sent every other status to `async_new_exception`, where `object_init_ex` wrote into a NULL bucket table: the process segfaulted at exit with its output already complete, which is why the only reproduction so far has been an intermittent one — true-async/server's `h2/060-h2-write-deadline-reclaims`, 4 failures in 30 standalone runs before this change and 0 in 30 after. `async_new_exception` now answers NULL while the executor is inactive, and the four call sites that released its result without a check take that NULL. Debug builds crashed; release builds took the fast-shutdown path, where the bucket table outlives the teardown and the exception was built into a store nothing reads again. A deterministic test needs the shape built on purpose, because `ZEND_ASYNC_IO_WRITEV_EX` has no caller inside php-src: `--enable-async-fuzz` builds queue such a write when `ASYNC_FUZZ_CANCELLED_WRITE=1` is set.
...
- **Cancelling a `ThreadPool` task's Future left the task running on the worker** (#233). `cancel()` rejected the Future on the submitting thread and nothing reached the worker at all: the task ran to its end, and everything it had captured stayed alive with it — a subscription, a connection, a file handle, held by a run whose caller had already given up. Measured: a task with `delay(800)`, cancelled at 200 ms, still printed its last line; a task cancelled while queued was executed in full. The shared state now carries a cancellation back to the producer, and the worker cancels the coroutine serving it — in both modes, and before unpacking the closure at all when the task is still queued. An awaiter that goes away without calling `cancel()` counts as one too, which is the common case: a cancelled scope or an expired timeout unwinds the coroutine and drops the Future. A Future nobody ever subscribed to is left alone, so `submit()` without awaiting keeps running as before, and `ignore()` opts out explicitly.
- **A `ThreadPool` task writing into `$GLOBALS` crashed the worker at shutdown** (#251). The key it stored was a literal from the task's own `op_array`, which lives in the snapshot arena and is freed with the task; the string is marked interned, so the symbol table kept it without a reference. While that memory stayed untouched the key still read as interned and nothing happened, so one such task was harmless; the next task reused the block, the header was overwritten, and shutdown handed the allocator a pointer it never issued — `zend_mm_heap corrupted`, then a crash. The same dangling key also broke the lookup, so the value written by one task read as absent in the next. A materialized closure no longer shares anything with the snapshot arena, which also makes a closure left in the worker's state callable from a later task. Two more of the same shape went with it: a literal array kept its keys and string values in the arena, so reading one back gave a garbage length, and the attribute table was left there whole, so reflection on a surviving closure read a freed header. The same localization covers the rest of what a materialized closure carries: parameter doc comments, the static variables of nested definitions, and defaults built from a constant expression. A function written into such an expression cannot be localized at all: a first-class callable or a partial application names a function resolved against the compiling thread, and a closure is an op_array the tree points at rather than owns, so it was left in the arena and the worker crashed on it once the arena was reused. A closure carrying either form is refused on transfer — in a parameter default and in an attribute argument alike.
- **The GC threshold grew by 10000 after every full root buffer in async mode and never came back down**, so collections ran further and further apart and the memory amplitude grew with them until a long-running server reached `memory_limit` (#234). A collection in async mode is handed to the GC coroutine and reports 0 collected nodes to the caller that triggered it, so the threshold is now adjusted inside that coroutine, from the count it actually collected.
- **A value set on `root_context()` was invisible to `current_context()->find()` in an HTTP handler.** The walk up the Scope chain ended at the first Scope holding no Context of its own, and a Scope receives a Context only when someone asks for one. A handler runs under a per-request Scope below the server's Scope, and neither of those two had ever touched its Context, so the walk stopped two links short of the main Scope while `root_context()` kept answering, because it reads the main Scope's Context directly (#237). Scopes without a Context are now skipped instead of ending the walk. Reproduced with main scope -> `Scope::inherit()` -> `Scope::inherit()` -> coroutine: `null` before the fix, the stored value after.
- **The Context stubs described a store that does not exist.** `Context` was documented as immutable with `set()` and `unset()` returning new instances, `has()` as reading the local Context alone, `get()` as reading it alone as well, and `current_context()` as the coroutine's own Context. `set()` and `unset()` change the Context in place and return it, `get()` and `has()` read the Scopes above too, and the coroutine's own Context comes from `coroutine_context()`. `Scope::__construct()` now carries a contract of its own: a `new Scope()` has no parent, so lookup from inside it never reaches the main Scope, `request_context()` is null there, and disposal cancels running coroutines until `allowZombies()` is called.
- **A value handed to a receiver was lost when that receiver was cancelled before it ran.** A wakeup only queues a coroutine, and the value stayed unguarded in the buffer meanwhile, so a cancellation arriving in between took the wakeup with it and left the value unreachable — `Async\DeadlockError` on a channel whose buffer still held the message. On a rendezvous channel `send()` had already reported delivery, so the message went missing without a trace. A value is now reserved for the receiver it wakes, and a receiver that leaves without taking it hands the reservation to the next one.
- **The same defect on the sender side left the channel deadlocked with a free slot.** `recv()` freed one slot and woke one parked sender; cancelling that sender before it ran left the other senders asleep and the next receiver parked on an empty channel. A freed slot is now reserved for the sender it wakes and handed on the same way.
- **Anything running between a wakeup and the coroutine it woke could take what had been promised to it.** `recv()`, `recvAsync()`, `sendAsync()` and `foreach` served themselves from the buffer regardless of who was already woken, so the woken coroutine found nothing and parked again. They now take only what is reserved for nobody. Two visible consequences: `recvAsync()` returns a pending Future where it used to return a resolved one, and `sendAsync()` returns false while the only free slot belongs to a woken sender. `count()`, `isEmpty()` and `isFull()` keep reporting the physical buffer, promised values and slots included, so `isFull() === false` does not guarantee a successful `sendAsync()`.
- **A `send()` that threw left its value in the channel for someone else to receive.** On a capacity-0 channel `send()` writes into the rendezvous slot before it parks; a cancellation arriving while it waited threw out of `send()` and left the value there, so the caller was told the message never went and a later `recv()` got it anyway — and a caller that retried sent it twice. A sender that leaves without hearing its value was taken now takes the value with it.
- **The slot freed by a rendezvous delivery was open to a newcomer for one tick.** Taking a value owes two answers: the sender hears its message went, and the sender already queued gets the slot that message frees. Only the first was paid at that moment and the second waited for the acknowledged sender to run, so a `send()` or `sendAsync()` arriving in between took the slot ahead of the one that had been waiting. Both are paid in the same call now.
- **A rendezvous sender was failed by a `close()` that arrived after its message had been taken.** `send()` on a capacity-0 channel parks until a receiver takes the value; the receiver taking it is the answer, but the sender only learns of it on a later tick, and a `close()` in between reached it first and raised `Async\ChannelException` on a message the receiver already held. A sender whose value has been taken now leaves the waiting queue at that moment, so nothing can fail it afterwards.
- **Waiters were not served in arrival order.** Removing a waiter moved the last element of the queue into the freed place, so the newest waiter was served next. Removal now shifts the tail, which is also what makes the handed-on reservation land on the oldest waiter. Deadlock reports name the reserved counts: `Channel(capacity=1, receivers=1, senders=0, reserved=1/0)`.
...
- **A transaction opened with raw SQL was handed to the next coroutine (#203).** The pool tracked transactions through PDO's own flag, which only `beginTransaction()` sets, so `START TRANSACTION` via `exec()` — and the implicit transaction under `autocommit=0` — left the slot returning to the pool mid-transaction, dirty rows and row locks included. The pool now asks the driver for the real state, which it answers from cached connection status without a round trip.
