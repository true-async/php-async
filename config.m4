...
if test "$PHP_ASYNC" = "yes"; then
  ...
  PHP_NEW_EXTENSION([async],
    [async.c coroutine.c scope.c scheduler.c exceptions.c iterator.c async_API.c \
     ...
     thread.c internal/allocator.c internal/circular_buffer.c \
     zend_common.c],
    $ext_shared)

  dnl Optionally install headers (if desired for public use).
  PHP_INSTALL_HEADERS([ext/async],
    [php_async.h coroutine.h scope.h scheduler.h exceptions.h iterator.h async_API.h context.h future.h channel.h pool.h task_group.h fs_watcher.h thread.h])
