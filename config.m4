...
if test "$PHP_ASYNC" = "yes"; then
  ...
  PHP_INSTALL_HEADERS([ext/async],
    [php_async.h coroutine.h scope.h scheduler.h exceptions.h iterator.h async_API.h context.h future.h channel.h thread_channel.h thread_pool.h pool.h task_group.h fs_watcher.h thread.h cpu_info.h])
    ...
    if test -x "$PKG_CONFIG" && $PKG_CONFIG --exists libuv; then
      dnl Require libuv >= 1.50.0 (uv_thread_detach; also covers the UV_RUN_ONCE busy loop fix)
      if $PKG_CONFIG libuv --atleast-version 1.50.0; then
        ...
        AC_MSG_ERROR(system libuv must be upgraded to version >= 1.50.0 (needs uv_thread_detach))
      fi
